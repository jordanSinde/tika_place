...
class TicketState {
  final Map<String, TicketData> tickets;
...
class TicketNotifier extends StateNotifier<TicketState> {
  ...
  Future<void> generateTickets(BookingState bookingState) async {
    ...
    try {
      ...
      final reference = bookingState.bookingReference!;

      // Générer un billet PDF pour chaque passager
      for (var i = 0; i < bookingState.passengers.length; i++) {
        final passenger = bookingState.passengers[i];
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  _buildPdfInfoSection(
                    'Informations du voyage',
                    [
                      _buildPdfRow('Départ', bus.departureCity),
                      _buildPdfRow('Arrivée', bus.arrivalCity),
                      _buildPdfRow('Date', bus.departureTime.toString()),
                      _buildPdfRow('Compagnie', bus.company),
                      _buildPdfRow('Bus N°', bus.registrationNumber),
                    ],
                  ),
                  pw.SizedBox(height: 20),
                  ...
                  _buildPdfInfoSection(
                    'Informations du passager',
                    [
                      _buildPdfRow('Nom', passenger.lastName),
                      _buildPdfRow('Prénom', passenger.firstName),
                      if (passenger.phoneNumber != null)
                        _buildPdfRow('Téléphone', passenger.phoneNumber!),
                      if (passenger.cniNumber != null)
                        _buildPdfRow('CNI', passenger.cniNumber.toString()),
                    ],
                  ),
                  pw.SizedBox(height: 20),

                  // QR Code (simulé avec un rectangle pour l'exemple)
                  pw.Center(
                    child: pw.Container(
                      width: 100,
                      height: 100,
      ...
      final filePath = '${output.path}/$reference.pdf';
      final file = File(filePath);
      await file.writeAsBytes(await pdf.save());

      // Mettre à jour l'état
      final newTickets = Map<String, TicketData>.from(state.tickets);
      newTickets[reference] = TicketData(
        reference: reference,
        filePath: filePath,
      ...
      state = state.copyWith(
        isGenerating: false,
        error: 'Erreur lors de la génération des billets: $e',
      );
    }
  }

  pw.Widget _buildPdfInfoSection(String title, List<pw.Row> rows) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
