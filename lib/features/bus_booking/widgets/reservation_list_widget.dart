...
class ReservationListWidget extends ConsumerWidget {
  final bool showOnlyActive;
  final int? limit;
  final Function(BookingStatus, {required int count})? onStatusCountChanged;
  final String? showOnly;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Notify about status counts if needed
    if (onStatusCountChanged != null) {
      _updateStatusCounts(ref);
    }
  ...
  void _updateStatusCounts(WidgetRef ref) {
    final reservations = ref.read(reservationProvider).reservations;
    final Map<BookingStatus, int> counts = {};
    ...
    for (final entry in counts.entries) {
      onStatusCountChanged!(entry.key, count: entry.value);
    }
  }
